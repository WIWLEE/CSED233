...
void task_1(ofstream &fout, const char *quiz_num) {
  ...
  int answer_1_2 = 7; // TODO: Change to your answer
...
/*
    ...
    Description:
        Print the depth and height of the specific node in a given tree
        TODO: Implement BinaryTree::getDepthHeight function
   ...
   the tree are unique)
    ...
    Output:
        - depth and height of the specific node in a given tree
...
void task_3(ofstream &fout, const char *tree_string, int node_value) {
  ...
  try {
    BinaryTree tree;
    tree.buildFromString(tree_string);
    fout << tree.getDepthHeight(node_value) << endl;
  } catch (const char *e) {
    cerr << e << endl;
  }
}
...
/*
    [Task 4] Properness, Fullness, Completeness of Binary Tree

    Description:
        Check whether if the given tree is proper, full, complete binary tree
        TODO:
        - Implement BinaryTree::isProper function
        - Implement BinaryTree::isFull function
        - Implement BinaryTree::isComplete function

    Input:
        - tree_string: string with bracket representation
        - check_char: characteristic you want to check, either proper, full,
   complete
    ...
    Output:
        ...
        - false otherwise
*/
void task_4(ofstream &fout, const char *tree_string, const char *check_char) {
  fout << "[Task 4]" << endl;
  try {
    BinaryTree tree;
    tree.buildFromString(tree_string);

    if (!strcmp(check_char, "proper")) {
      fout << tree.isProper() << endl;
    } else if (!strcmp(check_char, "full")) {
      fout << tree.isFull() << endl;
    } else if (!strcmp(check_char, "complete")) {
      fout << tree.isComplete() << endl;
    } else {
      cerr << "invalid property to check" << endl;
    }
  } catch (const char *e) {
    cerr << e << endl;
  }
...
/*
    ...
    Description:
        Delete the minimum value or the indexed value from the binary min heap.
        ...
        TODO: Implement MinHeap::deleteElement function

    Input:
        (‘insert’,integer): insert integer into the current min heap
        (‘delMin’,NULL): delete minimum value from current binary min heap
          and rearrange heap to maintain the min heap property.
        ('delete',index): delete the value indexed by the given index from the current binary min heap.
  ...
  try {
    ...
    for (int i = 0; i < instr_seq.getLength(); i++) {
      ...
      if (command.compare("insert") == 0) {
        ...
        heap.deleteElement(instr_seq.getInstruction(i).getValue());
