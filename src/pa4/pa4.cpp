...
/*
    ...
    Description:
        Implement a function that returns the number of cycles in the given undirected graph.
        A cycle is a non-empty path in which the only repeated vertices are the first and last vertices.
        Note that any subset of vertices of a cycle does not form another cycle (that is, do not count inner-cycles).
        You can modify graph.cpp and graph.h files for this problem.

        TODO: Implement Graph::addUndirectedEdge, which is implemented at the task 1, and Graph::countUndirectedCycle functions.
...
/*
    ...
    Description:
        Implement a function that performs a topological sort using the given directed graph.
        If there exists more than one result, print the topological sort that comes first in the ascending order.
        To take an example below, acceptable topological sorts are ‘A B C D F E’, ‘A C B F E D’, ‘A C D B F E’, etc.
        Among these, the desirable output is ‘A B C D F E’.
        Also, print ‘error’ if the topological sort could not be performed.
        You can modify graph.cpp and graph.h files for this problem.
...
/*
    ...
    Description:
        ...
        A cycle is a non-empty path in which the only repeated vertices are the first and last vertices.
        Unlike an undirected graph, the edges of the graph have direction in this time.
        For instance, (‘A’,’B’) is different from (‘B’,’A’).
        You can modify graph.cpp and graph.h files for this problem.

        TODO: Implement Graph::addDirectedEdge, which is implemented at the task 3, and Graph::countDirectedCycle functions.
